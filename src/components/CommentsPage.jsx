  ...
  return (
    <div className={`page-layer ${postId ? 'open' : ''}`} style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 16px',
        borderBottom:'1px solid #f0f0f0', flexShrink:0, position:'sticky', top:0,
        background:'#fff', zIndex:10 }}>
        <button onClick={onClose}
          style={{ width:34, height:34, borderRadius:'50%', background:'#f1f3f4', border:'none',
            cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontSize:15, color:'#5f6368' }}>
          <i className="fas fa-times"/>
        </button>
        <span style={{ fontWeight:700, fontSize:16, color:'#202124' }}>
      ...
      {post?.type === 'repost' && post.headline && (
        <div style={{ margin:'12px 16px 0', borderRadius:14, overflow:'hidden', border:'1px solid #e8eaed',
          ...
          {post.image && (
            <div style={{ position:'relative', width:'100%', paddingTop:'56.25%', background:'#f1f3f4', overflow:'hidden' }}>
              ...
              <div style={{ position:'absolute', top:10, left:10 }}>
                <span style={{ background:'rgba(0,0,0,.55)', backdropFilter:'blur(4px)', color:'#fff', fontSize:10,
          ...
          <div style={{ padding:'12px 14px', background:'#fff' }}>
            ...
            <p style={{ fontSize:15, fontWeight:700, color:'#202124', lineHeight:1.4, margin:0 }}>
              {post.headline}
            </p>
            <div style={{ marginTop:10, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              <span style={{ fontSize:11, color:'#9aa0a6' }}>{post.newsSource}</span>
      ...
      <div style={{ display:'flex', borderBottom:'1px solid #f0f0f0', background:'#fff',
        ...
        <button onClick={() => setTab('comments')}
          style={{ flex:1, padding:'11px 0', fontSize:13, fontWeight:600, border:'none', background:'none', cursor:'pointer',
            color: tab==='comments' ? '#1a73e8' : '#9aa0a6',
        ...
        {post?.type === 'repost' && (
          <button onClick={() => setTab('reposted')}
            style={{ flex:1, padding:'11px 0', fontSize:13, fontWeight:600, border:'none', background:'none', cursor:'pointer',
              color: tab==='reposted' ? '#34a853' : '#9aa0a6',
          ...
          <>
            {comments.length === 0 ? (
              <div style={{ textAlign:'center', padding:'48px 20px', color:'#9aa0a6' }}>
                <div style={{ width:56, height:56, borderRadius:'50%', background:'#f1f3f4',
                  display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 12px' }}>
                  <i className="far fa-comment" style={{ fontSize:22, opacity:.5 }}/>
                </div>
                <p style={{ fontWeight:600, marginBottom:4 }}>No comments yet</p>
                <p style={{ fontSize:13 }}>Be the first to comment!</p>
              </div>
            ) : (
              comments.map(c => (
                <div key={c.id} style={{ display:'flex', gap:10, padding:'12px 16px', borderBottom:'1px solid #f8f8f8' }}>
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:3 }}>
                      <span style={{ fontSize:13, fontWeight:700, color:'#202124' }}>{c.username}</span>
                      <span style={{ fontSize:11, color:'#9aa0a6' }}>{timeAgo(c.timestamp?.toDate?.() || c.timestamp)}</span>
                    </div>
                    <p style={{ fontSize:14, color:'#202124', lineHeight:1.5, margin:0 }}>{c.text}</p>
                  </div>
                </div>
              ))
            )}
          ...
          <>
            {repostedUsers.length === 0 ? (
              <div style={{ textAlign:'center', padding:'48px 20px', color:'#9aa0a6' }}>
                ...
                <p style={{ fontSize:13 }}>Be the first to share this news!</p>
              </div>
            ) : (
              <>
                <div style={{ padding:'10px 16px', background:'#f0f9f4', borderBottom:'1px solid #e6f4ea' }}>
                ...
                {repostedUsers.map((u, i) => (
                  <div key={i} style={{ display:'flex', gap:12, padding:'12px 16px', alignItems:'center',
                    ...
                    <div style={{ flex:1 }}>
                      <p style={{ fontSize:14, fontWeight:700, color:'#202124', margin:0 }}>@{u.username}</p>
                      <p style={{ fontSize:12, color:'#9aa0a6', margin:0 }}>{timeAgo(u.timestamp)}</p>
                    </div>
                    <i className="fas fa-chevron-right" style={{ color:'#d0d0d0', fontSize:12 }}/>
                  </div>
                ))}
              </>
            )}
      ...
      {tab === 'comments' && (
        <div style={{ padding:'10px 16px', borderTop:'1px solid #f0f0f0', display:'flex', gap:10,
          alignItems:'center', flexShrink:0, background:'#fff' }}>
          ...
          <input ref={inputRef} value={text} onChange={e => setText(e.target.value)}
            ...
            disabled={!user}
            style={{ flex:1, padding:'9px 14px', background:'#f1f3f4', border:'none', borderRadius:99,
