...
function RepostCard({ post, onClick }) {
  ...
  return (
    <div onClick={e => { e.stopPropagation(); onClick() }}
      style={{ borderRadius:14, overflow:'hidden', border:'1px solid #e8eaed', cursor:'pointer',
        marginTop:10, background:'#fff', boxShadow:'0 2px 10px rgba(0,0,0,.08)',
      ...
      {post.image && !imgErr ? (
        <div style={{ position:'relative', width:'100%', paddingTop:'56.25%', background:'#f1f3f4', overflow:'hidden' }}>
          ...
          <div style={{ position:'absolute', bottom:0, left:0, right:0, height:'50%', background:'linear-gradient(to top,rgba(0,0,0,.55),transparent)' }}/>
          ...
          <div style={{ position:'absolute', bottom:0, left:0, right:0, padding:'12px 14px' }}>
            <p style={{ color:'#fff', fontSize:14, fontWeight:700, lineHeight:1.4, margin:0,
              display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden', textShadow:'0 1px 4px rgba(0,0,0,.5)' }}>
              {post.headline}
            </p>
          </div>
        </div>
      ...
      <div style={{ padding:'10px 14px', background:'#fff', display:'flex', alignItems:'center', justifyContent:'space-between', gap:8 }}>
        <div style={{ display:'flex', alignItems:'center', gap:6, minWidth:0 }}>
          ...
          {post.newsCategory && (
            <span style={{ fontSize:10, color:'#1a73e8', fontWeight:700, background:'#e8f0fe', padding:'2px 7px', borderRadius:99, flexShrink:0, textTransform:'uppercase', letterSpacing:'.03em' }}>
              {post.newsCategory}
            </span>
          )}
        </div>
...
export default function PostCard({ post, id, onOpenComments, onOpenProfile, onAuthRequired, onMention, onHashtag }) {
  ...
  return (
    <article style={{ background:'#fff', borderRadius:12, padding:'14px 14px 10px',
      ...
      <div style={{ display:'flex', gap:10, alignItems:'flex-start', marginBottom:10 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
            ...
            {post.type === 'repost' && (
              <span style={{ fontSize:10, fontWeight:700, color:'#34a853', background:'#e6f4ea', padding:'1px 7px', borderRadius:4 }}>↺ Repost</span>
            )}
            {post.type === 'poll' && (
              <span style={{ fontSize:10, fontWeight:700, color:'#1a73e8', background:'#e8f0fe', padding:'1px 7px', borderRadius:4 }}>📊 Poll</span>
            )}
          </div>
      ...
      {post.type === 'repost' && (
        <RepostCard post={post} onClick={() => post.newsUrl && window.open(post.newsUrl, '_blank', 'noopener')}/>
      )}
      {post.type === 'text' && (
      ...
      <div style={{ display:'flex', alignItems:'center', gap:2, marginTop:10, paddingTop:8, borderTop:'1px solid #f5f5f5' }}
        ...
        <button style={{ display:'flex', alignItems:'center', gap:5, padding:'6px 10px', borderRadius:8, fontSize:13,
          color: isLiked ? '#e53935' : '#5f6368', background:'none', border:'none', cursor:'pointer', fontWeight:500 }}
        ...
        <button style={{ display:'flex', alignItems:'center', gap:5, padding:'6px 10px', borderRadius:8, fontSize:13,
          color:'#5f6368', background:'none', border:'none', cursor:'pointer', fontWeight:500 }}
          onMouseOver={e => e.currentTarget.style.background='#f8f9fa'}
          onMouseOut={e => e.currentTarget.style.background='none'}
        ...
        <button style={{ display:'flex', alignItems:'center', gap:5, padding:'6px 10px', borderRadius:8, fontSize:13,
          color:'#5f6368', background:'none', border:'none', cursor:'pointer', marginLeft:'auto' }}
          onMouseOver={e => e.currentTarget.style.background='#f8f9fa'}
          onMouseOut={e => e.currentTarget.style.background='none'}
          onClick={sharePost}>
