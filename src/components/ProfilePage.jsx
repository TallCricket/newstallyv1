  ...
  return (
    <div className={`page-layer ${uid ? 'open' : ''}`} style={{ background: 'var(--surface)' }}>
      ...
      {loading ? (
        ...
        <div style={{ padding: 40, textAlign: 'center', color: 'var(--muted)' }}>User not found</div>
      ) : (
        <div>
          {/* Cover */}
          ...
          <div style={{ padding: '0 16px 16px', background: 'var(--surface)' }}>
            ...
            {profile.bio && (
              <div style={{ fontSize: 14, color: 'var(--ink2)', marginBottom: 10, lineHeight: 1.5 }}>{profile.bio}</div>
            )}

            {/* Stats */}
            <div style={{ display: 'flex', gap: 24, marginTop: 12 }}>
              <div className="profile-stat">
                <span className="num">{formatCount(posts.length)}</span>
                <span className="lbl">Posts</span>
              </div>
              <div className="profile-stat">
                <span className="num">{formatCount(profile.followersCount || 0)}</span>
                <span className="lbl">Followers</span>
              </div>
              <div className="profile-stat">
                <span className="num">{formatCount(profile.followingCount || 0)}</span>
          ...
          <div style={{ borderTop: '1px solid var(--border)' }}>
            <div style={{ padding: '10px 16px 4px', background: 'var(--surface2)' }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.05em' }}>Posts</span>
            </div>
