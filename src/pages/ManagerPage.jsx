      ...
      <div style={{ width:'100%', maxWidth:380, background:'var(--surface)', borderRadius:20, padding:32, border:'1px solid var(--border)', boxShadow:'var(--shadow-lg)' }}>
        ...
        <form onSubmit={handleLogin}>
          <div style={{ marginBottom:14 }}>
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} required placeholder="manager email"
...
export default function ManagerPage() {
  ...
  useEffect(() => {
    if (activeTab === 'recent' && user?.email === MANAGER_EMAIL) loadRecent()
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      showToast('✅ Published! Appearing live in All category.')
      setForm(EMPTY_FORM)
    } catch(e) {
      console.error(e); showToast('Failed: ' + e.message)
    }
  ...
  const loadRecent = async () => {
    ...
    try {
      let snap
      try {
        snap = await getDocs(query(collection(db,'news'), orderBy('timestamp','desc'), limit(30)))
      } catch {
        snap = await getDocs(query(collection(db,'news'), limit(30)))
      }
  ...
  return (
    <div style={{ minHeight:'100dvh', background:'var(--bg)' }}>
      ...
      <div style={{ display:'flex', background:'var(--surface)', borderBottom:'1px solid var(--border)' }}>
        {[['add','➕ Publish News'],['recent','📋 Recent Posts']].map(([k,l]) => (
      ...
      {activeTab === 'add' && (
        <div style={{ maxWidth:640, margin:'0 auto', padding:'20px 16px 80px' }}>
          <div style={{ background:'rgba(26,115,232,.08)', border:'1px solid rgba(26,115,232,.2)', borderRadius:12, padding:'12px 16px', marginBottom:20, display:'flex', gap:10, alignItems:'flex-start' }}>
            <i className="fas fa-bolt" style={{ color:'#1a73e8', marginTop:1 }}/>
            <div>
              <div style={{ fontSize:13, fontWeight:700, color:'#1a73e8', marginBottom:2 }}>Live Updates</div>
              <div style={{ fontSize:12, color:'var(--muted)', lineHeight:1.5 }}>Published articles appear instantly in the <strong>All</strong> category on NewsTally for all users.</div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ background:'var(--surface)', borderRadius:14, padding:'18px 16px', marginBottom:16, border:'1px solid var(--border)' }}>
              <div style={{ fontSize:12, fontWeight:800, color:'var(--muted)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:14, display:'flex', alignItems:'center', gap:6 }}>
                <i className="fas fa-asterisk" style={{ color:'#ea4335', fontSize:10 }}/> Required
              </div>
              <div style={fieldStyle}>
                <label style={labelStyle}>Title *</label>
                <input value={form.title} onChange={e => set('title', e.target.value)} required placeholder="News headline" style={inputStyle}/>
              </div>
              <div style={fieldStyle}>
              ...
              <div style={fieldStyle}>
                ...
                <input value={form.source} onChange={e => set('source', e.target.value)} placeholder="e.g. NewsTally, DD News, ANI" style={inputStyle}/>
              </div>
            </div>

            <div style={{ background:'var(--surface)', borderRadius:14, padding:'18px 16px', marginBottom:16, border:'1px solid var(--border)' }}>
              <div style={{ fontSize:12, fontWeight:800, color:'var(--muted)', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:14 }}>Optional</div>
              <div style={fieldStyle}>
                <label style={labelStyle}>Description / Summary</label>
                <textarea value={form.description} onChange={e => set('description', e.target.value)} placeholder="Short summary..." rows={4} style={{ ...inputStyle, resize:'vertical', lineHeight:1.6 }}/>
              </div>
              <div style={fieldStyle}>
                <label style={labelStyle}>Article URL</label>
                <input type="url" value={form.url} onChange={e => set('url', e.target.value)} placeholder="https://example.com/article" style={inputStyle}/>
              </div>
              <div style={fieldStyle}>
                <label style={labelStyle}>Image URL</label>
                <input type="url" value={form.image} onChange={e => set('image', e.target.value)} placeholder="https://example.com/image.jpg" style={inputStyle}/>
                {form.image && <img src={form.image} alt="preview" style={{ width:'100%', height:140, objectFit:'cover', borderRadius:8, marginTop:8 }} onError={e => e.target.style.display='none'}/>}
              </div>
              <div style={fieldStyle}>
                <label style={labelStyle}>Publish Date & Time</label>
                <input type="datetime-local" value={form.pubDate ? new Date(form.pubDate).toISOString().slice(0,16) : ''} onChange={e => set('pubDate', e.target.value ? new Date(e.target.value).toISOString() : '')} style={inputStyle}/>
                <p style={{ fontSize:11, color:'var(--muted)', marginTop:4 }}>Leave blank to use current time</p>
              </div>
              <div style={fieldStyle}>
                <label style={labelStyle}>Author</label>
                <input value={form.author} onChange={e => set('author', e.target.value)} placeholder="Reporter / Author name" style={inputStyle}/>
              </div>
            </div>
            ...
            <button type="submit" disabled={saving || !form.title.trim()}
              ...
              {saving ? <><i className="fas fa-spinner fa-spin"/> Publishing...</> : <><i className="fas fa-paper-plane"/> Publish to NewsTally</>}
            </button>
          </form>
        </div>
      )}
      ...
      {activeTab === 'recent' && (
        <div style={{ maxWidth:640, margin:'0 auto', padding:'12px 16px 80px' }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
            <span style={{ fontSize:13, color:'var(--muted)', fontWeight:600 }}>{recentNews.length} articles</span>
            <button onClick={loadRecent} disabled={loadingRecent}
              ...
              <i className={`fas fa-sync-alt${loadingRecent?' fa-spin':''}`}/> Refresh
            </button>
          </div>
          {loadingRecent ? (
            ...
            <div style={{ textAlign:'center', padding:'60px 20px', color:'var(--muted)' }}>
              <i className="fas fa-newspaper" style={{ fontSize:40, display:'block', marginBottom:12, opacity:.3 }}/>
              <p style={{ fontWeight:700, color:'var(--ink)', marginBottom:6 }}>No articles yet</p>
              <p style={{ fontSize:13 }}>Published articles will appear here</p>
            </div>
          ) : recentNews.map(n => (
