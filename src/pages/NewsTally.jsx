...
// ── LocalStorage 2-stage cache (same as reference) ────────────────
const NT_CACHE_KEY  = 'nt_news_cache_v3'
const NT_CACHE_TIME = 'nt_news_cache_time_v3'
...
// ── Normalize a Firestore doc to consistent shape ─────────────────
function normalizeDoc(d) {
  const n = d.data ? d.data() : d
  // Resolve date from all possible fields
  const rawDate = n.savedAt || n.timestamp || n.pubDate || n.fetchedAt || n.date || ''
  let date = ''
  if (rawDate?.toDate)   date = rawDate.toDate().toISOString()
  else if (rawDate?.seconds) date = new Date(rawDate.seconds * 1000).toISOString()
...
// ── Sort: ranked articles first, then images-first, then newest ───
function sortNews(items) {
  return [...items].sort((a, b) => {
    const aRank = a.rank < 9999 ? a.rank : null
    const bRank = b.rank < 9999 ? b.rank : null
...
// ── Fetch first 20 fast, then 200 full ───────────────────────────
...
async function _fetch200(db_) {
  try {
    const snap = await getDocs(query(collection(db_, 'news'), orderBy('savedAt', 'desc'), limit(500)))
    if (!snap.empty) return snap.docs.map(normalizeDoc).filter(r => r.title)
  } catch {}
  try {
    const snap = await getDocs(query(collection(db_, 'news'), orderBy('timestamp', 'desc'), limit(500)))
    if (!snap.empty) return snap.docs.map(normalizeDoc).filter(r => r.title)
  } catch {}
  const snap = await getDocs(query(collection(db_, 'news'), limit(500)))
  return snap.docs.map(normalizeDoc).filter(r => r.title)
}
  ...
  useEffect(() => {
    getDoc(doc(db, 'config', 'rankings')).then(snap => {
      if (snap.exists() && Array.isArray(snap.data().categoryOrder)) {
        ...
        const cricket = order.filter(c => c.toLowerCase() === 'cricket')
        const others  = order.filter(c => c.toLowerCase() !== 'cricket')
        ...
        <div className="main-wrapper" style={{ paddingBottom:80 }}>
          ...
          {loading ? (
            ...
            <NewsLayout
              ...
              sentinelRef={sentinelRef} loadingMore={false}
